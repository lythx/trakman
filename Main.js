...
async function main () {
  ...
  ]).catch(err => ErrorHandler.fatal('Server authentication failed', err.errorString, err.errorCode))
  ...
  await Client.call('EnableCallbacks', [
    { boolean: true }
  ]).catch(err => { ErrorHandler.fatal('Failed to enable callbacks', err.errorString, err.errorCode) })
