...
export class Listeners {
  private static readonly listeners: tm.Listener[] = [{
    ...
    callback: async ([login, isSpectator]: tm.Events['TrackMania.PlayerConnect']): Promise<void> => {
      ...
      const player: tm.Player = await PlayerService.join(playerInfo.Login, playerInfo.NickName, playerInfo.Path,
        isSpectator, playerInfo.PlayerId, ip, playerInfo.OnlineRights === 3,
        playerInfo?.LadderStats.PlayerRankings[0]?.Score, playerInfo?.LadderStats.PlayerRankings[0]?.Ranking)
      AdministrationService.updateNickname({
        login,
        nickname: player.nickname
      })
      RecordService.updateInfo({
        login,
        nickname: player.nickname,
    ...
    callback: async ([playerId, login, timeOrScore, currentLap, checkpointIndex]: tm.Events['TrackMania.PlayerCheckpoint']): Promise<void> => {
    ...
    callback: async ([rankings, map, wasWarmUp, continuesOnNextMap, restart]: tm.Events['TrackMania.EndChallenge']): Promise<void> => {
      ...
      if (!config.keepQueueAfterLeave && nextJb.callerLogin !== undefined && PlayerService.get(
        nextJb.callerLogin) === undefined) {
    ...
    callback: (playerInfo: tm.Events['TrackMania.PlayerInfoChanged']): void => {
      ...
      const info: tm.InfoChangedInfo = {
        ...
        autoTarget: spec?.[spec.length - 4] === '1', // this is 255 when the player is spectating nobody OR when the player was spectating nobody
  ...
  static async initialize(): Promise<true | Error> {
    ...
    const cb: any | Error = await Client.call('EnableCallbacks', [{ boolean: true }])
