...
export abstract class TMXFetcher {
  ...
  private static readonly packmaskSite: tm.TMXSite = (config.manualMapLoading.stadiumOnly !== undefined ?
    config.manualMapLoading.stadiumOnly : process.env.SERVER_PACKMASK === 'nations') ? 'TMNF' : 'TMU'
   ...
   */
  static async fetchMapFile(mapId: string): Promise<{
    name: string,
    content: Buffer
  } | Error>
  /**
   ...
   * @returns Object containing map name and file content, or Error if unsuccessful
   */
  static async fetchMapFile(tmxId: number, site?: tm.TMXSite): Promise<{
    name: string,
    content: Buffer
  } | Error>
  static async fetchMapFile(id: number | string, site: tm.TMXSite = this.packmaskSite): Promise<{
    name: string,
    content: Buffer
  } | Error> {
    ...
    if (!res.ok) {
      const error = new Error(
        `Error while fetching map file from TMX` + ` (url: ${url}).\nCode: ${res.status} Text: ${res.statusText}`)
    ...
    const name: string = nameHeader[21] === '"' ? nameHeader.substring(22).split('"; filename*=')[0] :
      nameHeader.substring(21).split('; filename*=')[0]
   ...
   */
  private static async getTMXId(mapId: string): Promise<{
    id: number,
    prefix: TMXPrefix
  } | Error> {
    let data: any
    ...
    for (const p of this.prefixes) {
      ...
      const url = `https://${p}.tm-exchange.com/api/tracks?${new URLSearchParams([['fields', `TrackId`], ...params])}`
      const res = await fetch(url).catch((err: Error) => err)
      ...
      if (data instanceof Error) { // FOR WHATEVER REASON THE NEW API ALSO RETURNS A WEBPAGE INSTEAD OF AN ERROR (REAL HTTP 200 RESPONSE BY THE WAY!!!)
        const error = new Error(
          `Error while processing TMX response (url: ${url}).` + `\nCode: ${res.status} Text: ${res.statusText} `)
        Logger.warn(error.message)
        continue
      }
  ...
  static async fetchMapInfo(arg: string | number, prefix?: TMXPrefix | undefined): Promise<tm.TMXMap | Error> {
    ...
    if (typeof arg === 'number') {
      ...
      if (!res.ok) {
        const error = new Error(
          `Error while fetching map info from TMX` + ` (url: ${url}).\nCode: ${res.status} Text: ${res.statusText}`)
        Logger.warn(error.message)
        return error
      }
      data = await res.json().catch((data: Error) => data)
  ...
  static async getReplays(tmxId: number, prefix: TMXPrefix): Promise<tm.TMXReplay[] | Error> {
    ...
    if (res instanceof Error) {
      ...
      const error = new Error(
        `Error while fetching replays info from TMX (url: ${url}).` + `\nCode: ${res.status} Text: ${res.statusText}`)
      Logger.warn(error.message)
      return error
    } else {
      const rawData: any = await res.json().catch((data: Error) => data)
  ...
  static async searchForMap(query?: string, author?: string, site: tm.TMXSite = this.packmaskSite,
    ...
    if (!res.ok) {
      const error = new Error(
        `Error while searching for map on TMX (url: ${url}).` + `\nCode: ${res.status} Text: ${res.statusText} `)
      Logger.warn(error.message)
      return error
    }
   ...
   * @returns Object containing map name and file content, or Error if unsuccessful
   */
  static async fetchRandomMapFile(site: tm.TMXSite = this.packmaskSite): Promise<{
    name: string,
    content: Buffer
  } | Error> {
    const prefix = this.siteToPrefix(site)
