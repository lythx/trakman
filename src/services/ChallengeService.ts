...
export class ChallengeService {
  ...
  private static async initializeList(): Promise<void> {
    ...
    for (const c of challengesNotInDB) {
      ...
      const obj: TMChallenge = {
        id: info.UId, name: info.Name, fileName: info.FileName,
        author: info.Author, environment: info.Environnement,
        mood: info.Mood, bronzeTime: info.BronzeTime, silverTime: info.SilverTime,
        goldTime: info.GoldTime, authorTime: info.AuthorTime, copperPrice: info.CopperPrice,
        lapRace: info.LapRace, lapsAmount: info.NbLaps, checkpointsAmount: info.NbCheckpoints
      }
    ...
    for (const challenge of challengesInDB) {
      ...
      const info: TMChallenge = {
        ...
        author: c.author, environment: c.environment,
        mood: c.mood, bronzeTime: c.bronzetime, silverTime: c.silvertime,
        goldTime: c.goldtime, authorTime: c.authortime, copperPrice: c.copperprice,
        lapRace: c.laprace, lapsAmount: c.lapsamount, checkpointsAmount: c.checkpointsamount
      }
  ...
  static async add(fileName: string): Promise<TMChallenge | Error> {
    ...
    if (insert instanceof Error)
      return insert
    if (insert[0] === false)
      return new Error(`Failed to insert challenge ${fileName}`)
    const res = await Client.call('GetChallengeInfo', [{ string: fileName }])
    if (res instanceof Error)
      return res
    const info = res[0]
    const obj = {
      id: info.UId, name: info.Name, fileName: info.FileName, author: info.Author,
      environment: info.Environnement, mood: info.Mood, bronzeTime: info.BronzeTime,
      silverTime: info.SilverTime, goldTime: info.GoldTime, authorTime: info.AuthorTime,
      copperPrice: info.CopperPrice, lapRace: info.LapRace, lapsAmount: info.NbLaps,
      checkpointsAmount: info.NbCheckpoints
    }
