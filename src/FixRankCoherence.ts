...
import { MapRepository } from "./database/MapRepository"
...
export const fixCoherence = async () => {
    ...
    const players = await db.query(`SELECT player_ids.login, nickname, region, wins, time_played, visits, is_united, last_online, rank, average, privilege FROM players 
    JOIN player_ids ON players.id=player_ids.id`)
    const records = await repo.get(...maps.map(a => a.id))
