...
TM.dediRecords  -> `Returns an array of objects containing information about dedimania records on current challenge`
TM.getPlayerDedi() -> `Returns an object containing information about specified player's dedimania record on current map or undefined if the player has no record`
...
TM.callNoRes() -> `Calls a dedicated server method, doesn't expect any response`
TM.multiCall() -> `Calls multiple dedicated server methods using system.multicall. Throws error if the server responds with error, so you probably should catch it to avoid unnecessary crashes`
TM.multiCallNoRes() -> `Calls multiple dedicated server methods via system.multicall, doesn't expect any response`
...
TM.palette -> `Returns the current server palette as an object`
