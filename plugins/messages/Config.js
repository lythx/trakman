...
export default {
  ...
  welcome: `${p.server}» ${p.error}Welcome to ${p.highlight}#{name}${p.error}. `
    + `This server is running Trakman ${p.highlight}v#{version}${p.error}.`,
  ...
  win: `${p.server}» ${p.record}You have won your `
    + `${p.rank}#{wins}${p.record} race.`,
  leave: `${p.server}»» ${p.highlight}#{nickname}${p.servermsg} `
