...
const events: tm.Listener[] = [{
  ...
  callback: (): void => {
    if (c.startup.public) {
      tm.sendMessage(tm.utils.strVar(c.startup.message, { version: tm.config.controller.version }))
    }
  ...
  callback: async (info: tm.EndMapInfo): Promise<void> => {
    if (info.winnerLogin !== undefined && info.winnerWins !== undefined) {
      const player: tm.OfflinePlayer | undefined = tm.players.get(info.winnerLogin) ?? await tm.players.fetch(
        info.winnerLogin)
    ...
    if (tm.jukebox.juked[0]?.callerLogin !== undefined && c.nextJuke.public) {
      const player: tm.OfflinePlayer | undefined = tm.players.get(
        tm.jukebox.juked[0].callerLogin) ?? await tm.players.fetch(tm.jukebox.juked[0].callerLogin)
