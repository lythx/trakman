import { addManialinkListener, centeredText, componentIds, components, DynamicComponent, Grid, type GridCellFunction, icons, StaticHeader } from '../../ui/UI.js'
...
export default class BetPlaceWindow extends DynamicComponent {
  ...
  constructor() {
    ...
    addManialinkListener(this.id + config.actionIdOffset, config.options.length, (info, offset) => {
  ...
  private updatePosYAndHeight(): void {
    ...
    this.posY = config.staticPos.posY ?? (config.topBorder - data[side].slice(0, config.relativePos.widgetNumber)
  ...
  displayToPlayer(login: string, params: {
    ...
    const grid = new Grid(config.width + config.margin * 2, this.height + config.margin - StaticHeader.raceHeight,
      [1, 1, 1], [1, 1], {
    ...
    if (params.placedBet) {
      ...
      ${centeredText(`Prize: $${tm.utils.palette.green}${(this.prize ?? 0) * this.betLogins.length}C`, config.width,
        h / 2, {
          yOffset: h / 2 + config.prizeText.yOffset,
          textScale: config.prizeText.textScale
        })}`
    ...
    <manialink id="${this.id}">
      ...
      <frame posn="${config.posX} ${this.posY} -1">
        ...
        <frame posn="${headerW} 0 0">
          ...
          ${centeredText('$' + countdownColour + params.seconds.toString(), topRightW, StaticHeader.raceHeight,
