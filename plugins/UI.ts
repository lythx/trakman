...
const events: TMEvent[] = [
  ...
  },
  {
    ...
    callback: async (info: RecordInfo) => { //Should return TMRecord //record info contains both TMRecord and TMPlayer and some other stuf
      console.log(TM.players.length)
      for (const player of TM.players) {
        console.log(await TM.call('SendDisplayManialinkPageToLogin', [{ string: player.login }, { string: UIRace.buildLocalRecordsWidget(player) }, { int: 0 }, { boolean: false }]))
      }
    }
  },
  {
