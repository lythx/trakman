...
export default {
  ...
  allBestSectorsRemoved: `${p.admin}#{title} ${p.highlight}#{nickname}${p.admin} has removed ` + `${p.highlight}all sector records${p.admin} on the ongoing map.`,
  bestSectorRemoved: `${p.admin}#{title} ${p.highlight}#{nickname}${p.admin} has removed the ` + `${p.highlight}#{index}${p.admin} sector record on the ongoing map.`,
  commands: {
    deletesector: {
