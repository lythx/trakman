...
export default {
  ...
  chatPrefixEnabled: true, // Messages will be chosen randomly from this array
  messages: [`${p.highlight}This server is running ${p.rank}$L[${tm.config.controller.repo}]Trakman v${tm.config.controller.version}$L${p.highlight}.`,
    ...
    `${p.highlight}Use ${p.rank}/man ${p.highlight}with any command to see how to utilise it.`], // List of events to send a random message on, independently of the interval
  // For the list of events, peek the wiki
  // Alternatively, see the Events interface in /src/Namespace.ts/
  events: [`EndMap`, `adfadsfsdf`], // Whether to send a random message on a specific interval
