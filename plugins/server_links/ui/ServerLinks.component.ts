...
import {
    StaticComponent,
    StaticHeader,
    type StaticHeaderOptions,
    componentIds,
    Grid,
    type GridCellFunction,
    centeredText,
    leftAlignedText,
    Paginator,
} from '../../ui/UI.js'
...
export default class ServerLinks extends StaticComponent {
    ...
    <quad posn="0 0 1" sizen="${config.width - (headerCfg.squareWidth + config.margin) * (1 + buttonAmount)} ${headerCfg.height}" bgcolor="${headerCfg.textBackground}"/>
      ${leftAlignedText(config.title, config.width - (headerCfg.squareWidth + config.margin) * (1 + buttonAmount), headerCfg.height, {
    ...
    private constructEntry(ii: number, jj: number, ww: number, hh: number, startIndex: number): string {
        ...
        ${this.text(tm.utils.strip(data.serverName ?? data.name, false), w - iw - m, h, iw + m, config.iconBackground, true)}`
            },
            (i, j, w, h): string => {
                const width = 7
        ...
        return `<frame posn="${-config.margin} 0 2">${grid.constructXml(arr)}
      <quad posn="0 0 8"
           sizen="${ww + m * 2} ${hh + m}"
           image="${icons.blank}"
           imagefocus="${icons.join}"
           url="tmtp://#join=${data.login}"/>
           </frame>`
    }
