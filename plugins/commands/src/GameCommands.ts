...
const commands: tm.Command[] = [{
  ...
  params: [{
    name: 'mode',
    validValues: ['round', 'ta', 'team', 'lap', 'stunt', 'cup', 'rounds', 'timeattack', 'teams', 'laps', 'stunts']
  }],
  ...
  callback: (info: tm.MessageInfo, status: boolean, amount?: number): void => {
    ...
    tm.sendMessage(tm.utils.strVar(config.forceshowopp.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: status ? 'enabled' : 'disabled'
    }), config.forceshowopp.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, status: boolean): void => {
    tm.sendMessage(tm.utils.strVar(config.disablerespawn.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: status ? 'disabled' : 'enabled'
    }), config.disablerespawn.public ? undefined : info.login)
