...
const commands: tm.Command[] = [{
  ...
  callback: (info: tm.MessageInfo, password?: string): void => {
    ...
    tm.sendMessage(tm.utils.strVar(config.setrefpwd.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      password: password !== undefined ? password : 'none (disabled)'
    }), config.setrefpwd.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, mode: boolean): void => {
    tm.sendMessage(tm.utils.strVar(config.setrefmode.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      mode: mode ? 'ALL' : 'TOP3'
    }), config.setrefmode.public ? undefined : info.login)
  ...
  callback: async (info: tm.MessageInfo, amount: number, login?: string, message?: string): Promise<void> => {
    const status = await tm.utils.payCoppers(login ?? info.login, amount,
      message ?? tm.utils.strVar(config.pay.defaultMessage, {
        coppers: amount,
        server: tm.config.server.name
      }))
  ...
  callback: (info: tm.MessageInfo, name: string): void => {
    tm.sendMessage(tm.utils.strVar(config.setservername.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: name.length === 0 ? `the server login` : name
    }), config.setservername.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, comment: string): void => {
    tm.sendMessage(tm.utils.strVar(config.setcomment.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: comment.length === 0 ? `absolutely nothing` : comment
    }), config.setcomment.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, password?: string): void => {
    ...
    tm.sendMessage(tm.utils.strVar(config.setpassword.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: password !== undefined ? password : 'none (disabled)'
    }), config.setpassword.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, password?: string): void => {
    ...
    tm.sendMessage(tm.utils.strVar(config.setspecpassword.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: password !== undefined ? password : 'none (disabled)'
    }), config.setspecpassword.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, amount: number): void => {
    tm.sendMessage(tm.utils.strVar(config.setmaxplayers.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: amount
    }), config.setmaxplayers.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, amount: number): void => {
    tm.sendMessage(tm.utils.strVar(config.setmaxspecs.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: amount
    }), config.setmaxspecs.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, status: boolean): void => {
    tm.sendMessage(tm.utils.strVar(config.allowmapdownload.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: status ? 'allowed' : 'disallowed'
    }), config.allowmapdownload.public ? undefined : info.login)
  ...
  params: [{
    name: 'value',
    validValues: ['hidden', 'visible', 'notmnf']
  }],
  ...
  callback: (info: tm.MessageInfo, status: boolean): void => {
    tm.sendMessage(tm.utils.strVar(config.autosavereplays.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: status ? 'enabled' : 'disabled'
    }), config.autosavereplays.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo, status: boolean): void => {
    tm.sendMessage(tm.utils.strVar(config.autosavevalreplays.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      value: status ? 'enabled' : 'disabled'
    }), config.autosavevalreplays.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo): never => {
    tm.sendMessage(tm.utils.strVar(config.killcontroller.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname)
    }), config.killcontroller.public ? undefined : info.login)
    process.exit(0)
  },
  ...
  callback: (info: tm.MessageInfo, status: boolean): void => {
    tm.sendMessage(tm.utils.strVar(config.setbuddynotif.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      status: status ? 'enabled' : 'disabled'
    }), config.setbuddynotif.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo): void => {
    tm.sendMessage(tm.utils.strVar(config.shutdown.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname)
    }), config.shutdown.public ? undefined : info.login)
