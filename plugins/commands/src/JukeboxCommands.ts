...
const commands: tm.Command[] = [{
  ...
  callback: (info: tm.MessageInfo, index: number): void => {
    ...
    tm.sendMessage(tm.utils.strVar(config.dropjukebox.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname),
      name: tm.utils.strip(map.name)
    }), config.dropjukebox.public ? undefined : info.login)
  ...
  callback: (info: tm.MessageInfo): void => {
    ...
    tm.sendMessage(tm.utils.strVar(config.clearjukebox.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname)
    }), config.clearjukebox.public ? undefined : info.login)
  ...
  callback: async (info: tm.MessageInfo): Promise<void> => {
    tm.sendMessage(tm.utils.strVar(config.shuffle.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname)
    }), config.shuffle.public ? undefined : info.login)
    tm.jukebox.shuffle(info)
  },
  privilege: config.shuffle.privilege
}, {
  aliases: config.clearhistory.aliases,
  help: config.clearhistory.help,
  callback: async (info: tm.MessageInfo): Promise<void> => {
    tm.sendMessage(tm.utils.strVar(config.clearhistory.text, {
      title: info.title,
      adminName: tm.utils.strip(info.nickname)
    }), config.clearhistory.public ? undefined : info.login)
    tm.jukebox.clearHistory(info)
  },
  privilege: config.clearhistory.privilege
