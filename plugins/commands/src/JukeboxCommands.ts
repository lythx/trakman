...
const commands: tm.Command[] = [
  ...
  },
  {
    ...
    callback: async (info: tm.MessageInfo): Promise<void> => {
      tm.sendMessage(tm.utils.strVar(config.clearhistory.text, { title: info.title, adminName: tm.utils.strip(info.nickname) }), config.clearhistory.public ? undefined : info.login)
