...
export default class TopSums extends PopupWindow<number> {
  ...
  protected constructContent(login: string, page?: number): string {
    ...
    const firstCell: GridCellFunction = (i, j, w, h) => centeredText(
      '$' + config.colours.gold + this.ranks[getIndex(i, j)].sums[0].toString(), w, h)
    const secondCell: GridCellFunction = (i, j, w, h) => centeredText(
      '$' + config.colours.silver + this.ranks[getIndex(i, j)].sums[1].toString(), w, h)
    const thirdCell: GridCellFunction = (i, j, w, h) => centeredText(
      '$' + config.colours.bronze + this.ranks[getIndex(i, j)].sums[2].toString(), w, h)
    const otherCell: GridCellFunction = (i, j, w, h) => centeredText(this.ranks[getIndex(i, j)].sums[3].toString(), w,
      h)
