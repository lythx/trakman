...
export default class TopWins extends PopupWindow<number> {
  ...
  constructor() {
    super(componentIds.topWins, config.icon, config.title, config.navbar)
  ...
  protected constructContent(login: string, page?: number): string {
    ...
    const averageCell: GridCellFunction = (i, j, w, h) => centeredText(this.ranks[getIndex(i, j)].wins.toString(), w, h)
