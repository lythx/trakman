...
const initialize = async () => {
  const res: {
    ...
    nickname: string,
    visits: number
  }[] | Error = await tm.db.query(`SELECT login, nickname, visits FROM players
...
tm.addListener('PlayerJoin', (info) => {
  ...
  if (topList.length !== 0 && topList.length >= config.visitsCount && visits <= topList[topList.length - 1].visits) { return }
