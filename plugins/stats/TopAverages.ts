...
const initialize = async () => {
  const res: {
    ...
    nickname: string,
    average: number
  }[] | Error = await tm.db.query(`SELECT login, nickname, average FROM players
...
tm.addListener('RanksAndAveragesUpdated', async (info) => {
  ...
  for (const e of info) {
    if (topList.length !== 0 && topList.length >= config.averagesCount && e.average >= topList[topList.length - 1].average) { return }
    ...
    if (entry !== undefined) {
      ...
      const nickname: string | undefined = tm.players.get(e.login)?.nickname ?? (await tm.players.fetch(
        e.login))?.nickname
      const obj = {
        login: e.login,
