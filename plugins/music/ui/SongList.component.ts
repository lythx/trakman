import { addManialinkListener, centeredText, closeButton, componentIds, Grid, type GridCellFunction, Paginator, PopupWindow } from '../../ui/UI.js'
...
export default class SongList extends PopupWindow<DisplayParams> {
  ...
  private getPaginator(login: string, list: (Song & {
    index: number
    ...
    paginator.onPageChange = (login: string, page: number): Promise<void> => this.displayToPlayer(login, {
      ...
      paginator,
      list
    }, `${page}/${pageCount}`, privilege)
    return paginator
  }

  protected onOpen(info: {
    login: string,
    privilege: number
  }): void {
  ...
  private reRender(): void {
    ...
    for (const player of players) {
      ...
      this.displayToPlayer(player.login, player.params, `${page}/${paginator.pageCount}`,
        tm.players.get(player.login)?.privilege ?? 0)
    }
  }
  ...
  protected async constructContent(login: string, params?: DisplayParams, privilege = 0): Promise<string> {
    ...
    const headers: GridCellFunction[] = [(i, j, w, h) => centeredText(' Index ', w, h),
      (i, j, w, h) => centeredText(' Name ', w, h), (i, j, w, h) => centeredText(' Author ', w, h),
      (i, j, w, h) => centeredText('Queue position', w, h), (i, j, w, h) => centeredText(' Queued by ', w, h),
    ...
    const nameCell: GridCellFunction = (i, j, w, h) => centeredText(list[index + i].name, w, h)
    const authorCell: GridCellFunction = (i, j, w, h) => centeredText(list[index + i].author, w, h)
    ...
    const queuedByCell: GridCellFunction = (i, j, w, h) => centeredText(
      list[index + i].caller?.nickname ?? config.defaultText, w, h)
  ...
  private checkQueuePrivilege(login: string, privilege: number, song: Song): boolean {
    ...
    return !((privilege < pluginConfig.forceQueuePrivilege && (this.previousSongs.some(
      a => a.name === song.name) || this.songs.some(
      a => a.caller?.login === login))) || (song.isJuked && privilege < pluginConfig.forceQueuePrivilege))
  }
