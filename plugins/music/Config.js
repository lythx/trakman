...
export default {
  ...
  historySize: 3, // Privilege required to queue recently queued songs, queue more than one song and shuffle songs
  forceQueuePrivilege: 1,
  messages: {
    addToQueue: `${p.highlight}#{nickname} ${p.vote}has added ${p.highlight}#{song}${p.vote}` + ` by ${p.highlight}#{author}${p.vote} to the song queue.`,
    ...
    add: `${p.highlight}#{nickname} ${p.vote}has added and queued song ${p.highlight}#{song}${p.vote}` + ` by ${p.highlight}#{author}${p.vote}.`,
    addNameError: `${p.error}Song named ${p.highlight}#{name} ${p.error}already exists.`,
    addFileExtensionError: `${p.error}Song url file extension needs to be ` + `${p.highlight}.ogg ${p.error}or ${p.highlight}.mux${p.error}, ${p.highlight}#{url} ${p.error}is invalid.`,
