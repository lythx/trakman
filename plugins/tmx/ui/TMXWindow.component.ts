...
import { centeredText, closeButton, componentIds, Grid, type GridCellFunction, type GridCellObject, leftAlignedText, Paginator, PopupWindow } from '../../ui/UI.js'
...
class TMXWindow extends PopupWindow<number> {
  ...
  constructor() {
    ...
    this.paginator = new Paginator(this.openId, this.windowWidth, this.footerHeight,
      Math.ceil(config.queueCount / config.itemsPerPage))
  ...
  protected constructContent(login: string, page: number): string {
    ...
    const cell: GridCellFunction = (i, j, w, h) => {
      ...
      const screenshot: GridCellFunction = (ii, jj, ww, hh) => this.constructScreenshot(login, ww, hh, mapRecords,
        map.environment, TMXMap)
      const name: GridCellFunction = (ii, jj, ww, hh) => this.constructEntry(
        tm.utils.safeString(tm.utils.strip(tm.utils.decodeURI(mapName), false)), config.icons.name, ww, hh,
        config.iconWidth)
      ...
      const tmxRecords: GridCellFunction = (ii, jj, ww, hh) => this.counstructTmxRecordsXml(ww, hh,
        TMXMap?.validReplays)
      return grid.constructXml([header, screenshot, name, author, infos, tmxRecords])
    }
    return `<format textsize="3" /> ` + this.grid.constructXml(
      new Array(config.itemsPerPage).fill(null).map(() => cell))
  }
...
image="${image}" url="${url}" /> `
    ...
    if (TMXMap === undefined) {
      return `${this.constructEntry(title, config.icons.header, width - (config.iconWidth + config.margin), height,
        config.iconWidth)}
<frame posn="${width - (config.iconWidth + config.margin * 2)} 0 4" >
  ...
  private constructEntry(text: string, image: string, width: number, height: number, iconWidth: number,
    useCenteredText?: true): string {
  ...
  protected constructScreenshot(login: string, width: number, height: number, records: tm.Record[],
    environment: tm.Environment, TMXMap?: tm.TMXMap) {
    ...
    const indexCell: GridCellObject = {
      callback: (i, j, w, h) => {
        return centeredText(personalIndex === i ? (index + 1).toString() :
          (records[i - 1] === undefined ? config.defaultText : i.toString()), w, h, options)
      },
    ...
    const nameCell: GridCellFunction = (i, j, w, h) => {
      ...
      if (((records[i - 1] === undefined && i === 1) || (records[i - 1] === undefined && records[i - 2] !== undefined)) && (index > count || index === -1)) {
  ...
  protected constructAuthor(width: number, height: number, map: tm.Map): string {
    ...
    return `${this.constructEntry(tm.utils.safeString(author), config.icons.author, width - config.authorTimeWidth,
  ...
  private constructInfoXml(width: number, height: number, map: tm.Map, TMXMap?: tm.TMXMap): string {
    ...
    const cell: GridCellFunction = (i, j, w, h) => {
      ...
      <frame posn="${config.info.iconWidth + config.margin} 0 2">
        ...
        ${centeredText(infos?.[index]?.[0] ?? '', w - (config.info.iconWidth + config.margin), h,
        { textScale: config.info.textscale })}
      </frame>`
    }
    return grid.constructXml(
      new Array(config.info.columnsProportions.length * config.info.rows).fill(null).map(() => cell))
  }
  ...
  private counstructTmxRecordsXml(width: number, height: number, replays: tm.TMXReplay[] = []): string {
    ...
    const nameCell: GridCellFunction = (i, j, w, h) => leftAlignedText(
      tm.utils.safeString(replays[i - 1]?.name ?? config.defaultText), w, h, options)
    const dateCell: GridCellFunction = (i, j, w, h) => centeredText(
      replays[i - 1] !== undefined ? tm.utils.formatDate(replays[i - 1]?.recordDate, true) : config.defaultText, w, h,
      options)
