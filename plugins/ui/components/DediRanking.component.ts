...
export default class DediRanking extends StaticComponent implements IStaticComponent {
  ...
  private getWidgetContent(login: string) {
    ...
    const pr = TM.dediRecords.find(a => a.login === login)
    const prIndex = pr !== undefined ? TM.dediRecords.indexOf(pr) : Infinity
    for (const [i, p] of TM.dediRecords.entries()) {
      if (i < prIndex) {
        if (i >= CFG.dediRecordsWidget.topCount) {
      ...
      xml += // Records list in XML
        ...
        <label posn="6.1 ${(-1.8 * i) + 0.05} 0.04" sizen="${CFG.dediRecordsWidget.width - 5.7} 1.7" scale="0.9" 
      ...
      if (i === CFG.dediRecordsWidget.entries - 1) {
        break
      }
    }
    xml += '</frame>'
