...
export default class Navbar {
  ...
  constructor(buttons: { name: string, action: number }[], width: number, height: number | null = config.height,
  ...
  constructXml(): string {
    ...
    for (const [i, e] of this.buttons.entries()) {
      xml += `<frame posn="${w * i} 0 1">
            <quad posn="0 0 3" sizen="${w - config.margin} ${this.height}" image="f" imagefocus="${this.hoverImage}" action="${e.action}"/>
