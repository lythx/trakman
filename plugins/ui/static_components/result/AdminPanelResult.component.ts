...
export default class AdminPanelResult extends StaticComponent {
  ...
  constructor() {
    ...
    this.grid = new Grid(config.width + config.margin * 2, config.height - this.header.options.height,
      new Array(5).fill(1), [1], { margin: config.margin })
  ...
  display() {
    ...
    return [{
      ...
      xml: this.noShuffleXml,
      login: noShuffleLogins
    }, {
      xml: `<manialink id="${this.id}"></manialink>`,
      login: noPrivilegeLogins
    }]
  }
  ...
  private constructXml(noShuffle?: boolean): string {
    ...
    const jukeboxButton: GridCellFunction = (i, j, w, h): string => this.constructButton(w, h, config.icons.jukebox,
      config.iconsHover.jukebox, this.actions.jukebox)
    return `
