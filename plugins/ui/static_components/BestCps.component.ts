...
export default class BestCps extends StaticComponent {
  ...
  private constructText(login: string): string {
    ...
    const indexCell = (i: number, j: number, w: number, h: number): string => {
      return this.bestCps[i] === undefined ? '' : centeredText(`${i + 1}.`, w, h, { textScale: 0.87, padding: 0.1 })
    }
    ...
    const timeCell = (i: number, j: number, w: number, h: number): string => {
      ...
      return format + centeredText(TM.Utils.getTimeString(this.bestCps[i].time), w, h, { textScale: 0.87, padding: 0.1 })
    }

    const nicknameCell = (i: number, j: number, w: number, h: number): string => {
      return this.bestCps[i] === undefined ? '' : verticallyCenteredText(TM.strip(this.bestCps[i].nickname, false), w, h, { textScale: 0.87, padding: 0.2 })
    }
