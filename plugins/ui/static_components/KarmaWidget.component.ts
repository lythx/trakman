...
export default class KarmaWidget extends StaticComponent {
  ...
  private readonly buttonW = 1.7
  private readonly options = [-3, -2, -1, 1, 2, 3]
  private xml: string = ''
  ...
  private updateXML(): void {
    ...
    const karma = (voteAmounts.reduce((acc, cur, i) => acc += cur * (this.options[i] / 3)) / totalVotes) * 100
  ...
  private constructGraph(maxAmount: number, voteAmounts: number[]) {
    const width = (this.width + this.margin - this.buttonW) / 2 - this.margin
    const colours = ['F00A', 'D00A', 'BOOA', '0B0A', '0D0A', '0F0A']
    ...
    const h = (this.height - (this.headerH + this.margin * 2)) / this.options.length
  ...
  private constructInfo(totalVotes: number, karma: number, voteAmounts: number[]) {
    const height = this.height - this.headerH
    const width = (this.width + this.margin - this.buttonW) / 2 - this.margin
    const colour = karma > 0 ? '$F00' : '$0F0'
    const grid = new Grid(width, height, new Array(3).fill(1), new Array(3).fill(1))
  ...
  private constructButtons() {
    let ret = `<quad posn="0 0 1" sizen="${this.buttonW} ${this.height - (this.headerH + this.margin)}" bgcolor="${CONFIG.static.bgColor}"/>`
    const options = ['---', '--', '-', '+', '++', '+++']
    const colours = ['F00A', 'D00A', 'BOOA', '0B0A', '0D0A', '0F0A']
    const h = (this.height - (this.headerH + this.margin * 2)) / options.length
