...
export default abstract class PopupWindow extends DynamicComponent {
  ...
  private readonly margin1 = CONFIG.static.marginSmall

  constructor(windowId: number, windowHeight: number = 60, windowWidth: number = 80) {
    ...
    this.contentHeight = windowHeight - (2 * this.titleHeight)
    this.windowHeight = windowHeight + (2 * this.titleHeight)
    ...
    return [
      ...
      `
          ...
          <frame posn="0 -${this.titleHeight} 5">
            <quad posn="0 0 0" sizen="${this.contentWidth} ${this.contentHeight - CONFIG.static.marginSmall}" bgcolor="0009"/>
            <frame posn="0 0 1">`,
      `
          </frame>
        </frame>
