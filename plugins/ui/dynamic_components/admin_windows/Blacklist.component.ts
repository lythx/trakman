...
export default class Blacklist extends PopupWindow<number> {
  ...
  protected async constructContent(login: string, page: number = 1): Promise<string> {
    ...
    const headers: GridCellFunction[] = [(i, j, w, h) => centeredText(' Index ', w, h),
      ...
      (i, j, w, h) => centeredText(' Admin ', w, h), (i, j, w, h) => centeredText(' Unblacklist ', w, h)]
    ...
    const dateCell: GridCellFunction = (i, j, w, h) => centeredText(
      tm.utils.formatDate(blacklist[i + index].date, true), w, h)
    ...
    const adminCell: GridCellFunction = (i, j, w, h) => centeredText(
      tm.utils.safeString(tm.utils.strip(blacklist[i + index].callerNickname, false)), w, h)
