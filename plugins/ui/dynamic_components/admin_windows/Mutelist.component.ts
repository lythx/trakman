...
export default class Mutelist extends PopupWindow<number> {
  ...
  protected async constructContent(login: string, page: number = 1): Promise<string> {
    ...
    const headers: GridCellFunction[] = [(i, j, w, h) => centeredText(' Index ', w, h),
      ...
      (i, j, w, h) => centeredText(' Date ', w, h), (i, j, w, h) => centeredText(' Reason ', w, h),
      (i, j, w, h) => centeredText(' Admin ', w, h), (i, j, w, h) => centeredText(' Unmute ', w, h)]
    ...
    const loginCell: GridCellFunction = (i, j, w, h) => mutelist[i + index].login === login ?
      centeredText('$' + config.selfColour + mutelist[i + index].login, w, h) :
      centeredText(mutelist[i + index].login, w, h)
    const dateCell: GridCellFunction = (i, j, w, h) => centeredText(tm.utils.formatDate(mutelist[i + index].date, true),
      w, h)
    ...
    const adminCell: GridCellFunction = (i, j, w, h) => centeredText(
      tm.utils.safeString(tm.utils.strip(mutelist[i + index].callerNickname, false)), w, h)
