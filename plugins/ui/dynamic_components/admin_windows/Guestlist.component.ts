...
export default class Guestlist extends PopupWindow<number> {
  ...
  protected async constructContent(login: string, page: number = 1): Promise<string> {
    ...
    const headers: GridCellFunction[] = [(i, j, w, h) => centeredText(' Index ', w, h),
      ...
      (i, j, w, h) => centeredText(' Date ', w, h), (i, j, w, h) => centeredText(' Admin ', w, h),
    ...
    const loginCell: GridCellFunction = (i, j, w, h) => guestlist[i + index].login === login ?
      centeredText('$' + config.selfColour + guestlist[i + index].login, w, h) :
      centeredText(guestlist[i + index].login, w, h)
    const dateCell: GridCellFunction = (i, j, w, h) => centeredText(
      tm.utils.formatDate(guestlist[i + index].date, true), w, h)
