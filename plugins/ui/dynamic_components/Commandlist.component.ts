...
export default class CommandList extends PopupWindow<DisplayParams> {
  ...
  constructor() {
    ...
    tm.addListener("PrivilegeChanged", (info) => {
      ...
      } | undefined = this.getPlayersWithWindowOpen(true).find(a => a.login === info.login)
      if (p !== undefined) {
        ...
        if (info.newPrivilege < p.params.privilege || p.params.singleType === undefined) {
          ...
          this.displayToPlayer(info.login, {
            ...
            commands,
            paginator,
            privilege: info.newPrivilege
          }, `${page}/${paginator.pageCount}`, info.newPrivilege)
        } else {
          this.displayToPlayer(info.login, {
            ...p.params,
            privilege: info.newPrivilege
          }, `0/${p.params.paginator.pageCount}`, info.newPrivilege)
        }
      }
    })
    tm.commands.add({
  ...
  private initializeListsAndPaginators() {
    ...
    this.userPaginator = new Paginator(this.openId + 100, this.contentWidth, this.footerHeight,
      Math.ceil(this.userCommands.length / config.entries))
    this.opPaginator = new Paginator(this.openId + 200, this.contentWidth, this.footerHeight,
      Math.ceil(this.opCommands.length / config.entries))
    this.adminPaginator = new Paginator(this.openId + 300, this.contentWidth, this.footerHeight,
      Math.ceil(this.adminCommands.length / config.entries))
    this.masteradminPaginator = new Paginator(this.openId + 400, this.contentWidth, this.footerHeight,
      Math.ceil(this.masteradminCommands.length / config.entries))
    this.ownerPaginator = new Paginator(this.openId + 500, this.contentWidth, this.footerHeight,
      Math.ceil(this.ownerCommands.length / config.entries))
    const paginators = [this.userPaginator, this.opPaginator, this.adminPaginator, this.masteradminPaginator,
      this.ownerPaginator]
    ...
    for (const e of paginators) {
      e.onPageChange = (login: string, page: number): void => {
        ...
        this.displayToPlayer(login, {
          page,
          paginator: e,
          commands: commands[paginators.indexOf(e)],
          privilege,
          singleType: true
    ...
    for (let i = 0; i <= 4; i++) {
      const arr = [this.userCommands, this.opCommands, this.adminCommands, this.masteradminCommands,
        this.ownerCommands].slice(0, i + 1)
      ...
      const paginator: Paginator = new Paginator(this.openId + (i * 10), this.contentWidth, this.footerHeight,
        pageCount)
    ...
    this.table = new Grid(this.contentWidth, this.contentHeight, config.columnProportions,
  ...
  openWithQuery(login: string, privilege: number, query: string) {
    ...
    const list: tm.Command[] = [...indices.map(a => commands[a]), ...matchedAliases].filter(
    ...
    const paginator = new Paginator(paginatorId, this.contentWidth, this.footerHeight,
      Math.ceil(list.length / config.entries))
    paginator.onPageChange = (login: string, page: number, info): void => {
      this.displayToPlayer(login, {
        page,
        paginator,
  ...
  protected constructContent(login: string, params: DisplayParams): string {
    ...
    const headers: GridCellFunction[] = [(i, j, w, h) => centeredText(' P ', w, h),
      (i, j, w, h) => centeredText(' Aliases ', w, h), (i, j, w, h) => centeredText(' Arguments ', w, h),
    ...
    const privCell: GridCellFunction = (i, j, w, h) => {
      ...
      const text: string = config.slashesForPrivilege ? (command.privilege > 0 ? '//' : '/') :
        command.privilege.toString()
  ...
  protected constructFooter(login: string, params: DisplayParams): string {
    return closeButton(this.closeId, this.windowWidth, this.headerHeight - this.margin) + params.paginator.constructXml(
      params.page)
  }
