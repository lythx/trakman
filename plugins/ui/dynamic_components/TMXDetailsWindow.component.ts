...
export default class TMXDetailsWindow extends PopupWindow {
    ...
    private constructDescription(width: number, height: number, map: tm.TMXMap): string {
        ...
        const w = width - p * 4
        const h = height - p * 4
        const description = (map.comment.trim())
        ...
        <frame posn="${p * 2} ${-p * 2} 1">
          ...
          text="${config.description.format}${description.length === 0
                ? config.description.defaultText
                : description.length > config.description.textLength
                    ? description.replace(/\[(.*?)\]|\[\/(.*?)\]/gi, '').substring(0, config.description.textLength) + '...'
                    : description
            }" autonewline="1"/>
        </frame>`
    }
