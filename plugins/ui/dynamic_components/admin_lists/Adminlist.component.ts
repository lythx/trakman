...
import config from './Adminlist.config.js'
import { actions } from '../../../actions/Actions.js'

export default class Adminlist extends PopupWindow<{ page: number, privilege: number }> {

	readonly grid: Grid
	readonly paginator: Paginator

	constructor() {
		super(componentIds.adminlist, config.icon, config.title, config.navbar, config.width)
		...
		this.paginator = new Paginator(this.openId, this.contentWidth, this.footerHeight, Math.ceil(tm.admin.adminCount / (config.entries - 1)))
		...
		addManialinkListener(this.openId + 1000, 1000, async (info, offset) => {
			const target = tm.admin.adminlist[offset]
		...
		tm.addListener('PrivilegeChanged', () => {
			this.paginator.setPageCount(Math.ceil(tm.admin.adminCount / config.entries))
			this.reRender()
		})
		tm.commands.add({
	...
	protected async constructContent(login: string, params: { page: number, privilege: number }) {
		...
		const adminlist = tm.admin.adminlist
		const fetchedPlayers = await tm.players.fetch(adminlist.map(a => a.login))
		...
		const nicknameCell: GridCellFunction = (i, j, w, h) => {
			const nickname = fetchedPlayers.find(a => a.login === adminlist[i + index].login)?.nickname
			return centeredText(tm.utils.safeString(tm.utils.strip(nickname ?? config.defaultNickname, false)), w, h)
		}
		const loginCell: GridCellFunction = (i, j, w, h) => adminlist[i + index].login === login ?
			centeredText('$' + config.selfColour + adminlist[i + index].login, w, h) : centeredText(adminlist[i + index].login, w, h)
    ...
    imagefocus="${config.demoteIconHover}" halign="center" valign="center"${actionStr}/> `
		...
		const rows = Math.min((config.entries - 1), adminlist.length - (index + 1))
		const arr = headers
