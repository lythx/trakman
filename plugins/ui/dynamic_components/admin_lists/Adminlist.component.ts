...
export default class Adminlist extends PopupWindow<{
  page: number,
  privilege: number
}> {
  ...
  constructor() {
    ...
    this.paginator = new Paginator(this.openId, this.contentWidth, this.footerHeight,
      Math.ceil(tm.admin.adminCount / (config.entries - 1)))
  ...
  protected async constructContent(login: string, params: {
    ...
    const loginCell: GridCellFunction = (i, j, w, h) => adminlist[i + index].login === login ?
      centeredText('$' + config.selfColour + adminlist[i + index].login, w, h) :
      centeredText(adminlist[i + index].login, w, h)
