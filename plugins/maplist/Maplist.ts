...
tm.addListener('Startup', (): void => {
  const arr1: TM.Map[] = tm.maps.list.sort((a, b): number => a.name.localeCompare(b.name))
...
export const maplist = {
  ...
  get: (sort?: 'name' | 'karma' | 'long' | 'short' | 'worstkarma' | 'bestkarma'): readonly Readonly<TM.Map>[] => {
    switch (sort) {
      case 'name':
        return nameSort
