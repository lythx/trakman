...
export default {
  ...
  allBestCpsRemoved: `${p.admin}#{title} ${p.highlight}#{nickname}${p.admin} has removed ` + `${p.highlight}all checkpoint records${p.admin} on the ongoing map.`,
  bestCpRemoved: `${p.admin}#{title} ${p.highlight}#{nickname}${p.admin} has removed the ` + `${p.highlight}#{index}${p.admin} checkpoint record on the ongoing map.`,
  commands: {
    deletecp: {
