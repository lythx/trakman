...
export const allCpsDB = {
  ...
  async get(mapDBId: number, ...playerLogins: string[]): Promise<{
    ...
    return res.map(a => ({
      login: a.login,
      nickname: a.nickname,
      checkpoints: a.checkpoints.map(b => b === -1 ? undefined : b)
    }))
  },
...
export const bestCpsDB = {
  ...
  async get(mapId: string | number): Promise<{
    ...
    const res: {
      ...
      login: string,
      nickname: string
    }[] | Error = (await queryDB(`SELECT checkpoint, index, date, login, nickname FROM best_checkpoint_records 
  ...
  async add(mapId: string | number, playerId: string | number, index: number, checkpoint: number,
  ...
  async update(mapId: string | number, playerId: string | number, index: number, checkpoint: number,
    date: Date): Promise<void> {
    const mapDBId = typeof mapId === 'number' ? mapId : await tm.db.getMapId(mapId)
    const playerDBId = typeof playerId === 'number' ? playerId : await tm.db.getPlayerId(playerId)
    if (mapDBId === undefined || playerDBId === undefined) { return }
