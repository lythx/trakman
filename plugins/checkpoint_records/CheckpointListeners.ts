...
import { allCpsDB, bestCpsDB } from './CheckpointDB.js'
...
if (config.isEnabled) {
  ...
  tm.addListener('PlayerCheckpoint', (info: tm.CheckpointInfo) => {
    ...
    if (playerCheckpoints === undefined) {
      currentPlayerCps.push({
        login: info.player.login,
        nickname: info.player.nickname,
        checkpoints: [info.time]
      })
      void allCpsDB.add(currentMapDBId, info.player.login, [info.time])
      emitEvent('PlayerCheckpoint', {
        login: info.player.login,
        nickname: info.player.nickname,
        index: info.index
      })
    } else if (((playerCheckpoints.checkpoints[info.index] ?? Infinity) > info.time && !isStunts) || ((playerCheckpoints.checkpoints[info.index] ?? -1) < info.time && isStunts)) {
      playerCheckpoints.checkpoints[info.index] = info.time
      void allCpsDB.update(currentMapDBId, info.player.login,
        playerCheckpoints.checkpoints.map(a => a === undefined ? -1 : a))
      emitEvent('PlayerCheckpoint', {
        login: info.player.login,
        nickname: info.player.nickname,
        index: info.index
  ...
  tm.addListener('PlayerJoin', async (info) => {
    ...
    if (playerCps instanceof Error) {
      await tm.log.fatal(`Failed to fetch player ${info.login} checkpoint records for map ${tm.maps.current.id}`,
        playerCps.message)
      return
    }
    currentPlayerCps.push(...playerCps)
  ...
  tm.commands.add({
    ...
    callback(info, cpIndex?: number) {
      ...
      if (cpIndex === undefined) {
        ...
        emitEvent('DeletePlayerCheckpoint', {
          ...info,
          deletedCheckpoints: arr.map((a, i) => ({
            time: a,
            index: i
          }))
        })
        ...
        tm.sendMessage(tm.utils.strVar(config.playerCpRemoved, { index: tm.utils.getOrdinalSuffix(cpIndex) }),
          info.login)
    ...
    callback(info, cpIndex?: number) {
      if (cpIndex === undefined) {
        ...
        emitEvent('DeleteBestCheckpoint', arr.map((a, i) => ({
          ...a,
          index: i
        })))
        ...
        if (deleted !== undefined) {
          emitEvent('DeleteBestCheckpoint', [{
            ...deleted,
            index: cpIndex
          }])
        }
...
const getMapCheckpoints = (): ({
  ...
  const arr: ({
    ...
    checkpoint: number,
    date: Date
  } | null)[] = new Array(tm.maps.current.checkpointsAmount - 1).fill(null)
...
const getPlayerCheckpoints = (): ({
  ...
  for (const [i, e] of tm.players.list.entries()) {
    arr[i] = {
      login: e.login,
      nickname: e.nickname,
      checkpoints: new Array(tm.maps.current.checkpointsAmount - 1).fill(null)
      .map((a, i) => currentPlayerCps.find(a => a.login === e.login)?.checkpoints[i] ?? null)
    }
  }
  return arr
}
